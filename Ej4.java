/*
 * Escribe un programa que muestre tu horario de clase.
 * 
 * @author KuzaFkto
 */

public class Ej4{
    public static void main(String[] args) {
        
            System.out.printf(" %-17s %-17s %-17s %-17s %-17s %-17s",""," \033[34m Lunes","Martes","Miercoles","Jueves","Viernes");
            System.out.println("");
            System.out.printf("--------------------------------------------------------------------------------------------------");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","8:15","\033[31m Programación","Sistemas","Entornos","Programación","FOL");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","9:15","Programación","Sistemas","Entornos","Programación","FOL");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","10:15","Lenguajes","Sistemas","Entornos","Programación","FOL");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","11:15","Lenguajes","Sistemas","Programación","Lenguajes","Sistemas");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","Recreo","Recreo","Recreo","Recreo","Recreo","Recreo");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","11:45","Lenguajes","Programación","Programación","Lenguajes","Sistemas");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","12:45","BBDD","Programación","BBDD","BBDD","Sistemas");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","13:45","BBDD","Entornos","BBDD","BBDD","Sistemas");
            System.out.printf("%-17s %-17s %-17s %-17s %-17s %-17s\n","14:45","BBDD","Entornos","BBDD","BBDD","Sistemas");
        


    }
}